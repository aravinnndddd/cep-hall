...
const AdminPanel: React.FC = () => {
  ...
  const handleApprove = async (booking: Booking) => {
    ...
    try {
      ...
      } else if (approver.role === "staff" && booking.status === "waiting_staff") {
      ...
      } else if (approver.role === "principal" && booking.status === "waiting_principal") {
  ...
  const isAuthorizedToApprove = (booking: Booking) => {
    ...
    if (approver.role === "hod") {
      return booking.status === "waiting_hod" && approver.department === booking.department;
    }
    if (approver.role === "staff") {
      return booking.status === "waiting_staff" && (!approver.resourceId || approver.resourceId === booking.resourceId);
    }
    ...
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 bg-white border border-zinc-200 p-1 rounded-xl overflow-x-auto">
          {(["all", "waiting_hod", "waiting_staff", "waiting_principal", "approved", "correction_allowed", "rejected"] as const).map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-wrap gap-4 text-[10px] font-bold uppercase tracking-widest">
                    <div className={`flex items-center gap-1.5 ${booking.hodApproved ? "text-emerald-600" : "text-zinc-400"}`}>
                      <div className={`w-2 h-2 rounded-full ${booking.hodApproved ? "bg-emerald-500" : "bg-zinc-300"}`} />
                      HOD {booking.hodApproved && `(${booking.hodEmail})`}
                    </div>
                    <div className={`flex items-center gap-1.5 ${booking.staffApproved ? "text-emerald-600" : "text-zinc-400"}`}>
                      <div className={`w-2 h-2 rounded-full ${booking.staffApproved ? "bg-emerald-500" : "bg-zinc-300"}`} />
                      Staff {booking.staffApproved && `(${booking.staffEmail})`}
                    </div>
                    <div className={`flex items-center gap-1.5 ${booking.principalApproved ? "text-emerald-600" : "text-zinc-400"}`}>
                      <div className={`w-2 h-2 rounded-full ${booking.principalApproved ? "bg-emerald-500" : "bg-zinc-300"}`} />
                      Principal {booking.principalApproved && `(${booking.principalEmail})`}
                    </div>
                  </div>
